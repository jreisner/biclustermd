...
#' @param obj A matrix for row prototypes, typically named Q
...
#' @return A matrix for row prototypes, i.e., a Q matrix.
...
random_assign_unassigned_fill_empties_Q <- function(data, obj,
  ...
  if(length(unassigned) == 0 & length(empty_protos) == 0) {
    ...
    for(j in 1:num_to_fill) {
      ...
      mean_row <- mean(rowMeans(data[chosen_proto_members,], na.rm = TRUE), na.rm = TRUE)
      row_means <- rowMeans(data[chosen_proto_members,], na.rm = TRUE)
